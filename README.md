...
- 环形视图保持 CPU / RAM / Disk 横排，压缩网络明细间距；单节点分组可使用完整行宽，宽卡片的网络与三网信息并排展示。
- 团子采用固定正圆底座，完整保留五只团子，不拉伸图片。
- 支持半透明磨砂表面、非线性缓动与页脚缓慢流光；不支持背景模糊时保留不透明底色。系统开启减少动态效果时，同时停用 CSS 与详情图表动画。
- 验证：320 / 390 / 768 / 1440px、浅色与深色、条形与环形视图；29 项 Chromium 浏览器测试、1 项主题契约测试、37 项项目测试通过，16 份 axe 检查无违规。测试使用本地演示数据，不代表真实节点状态。
- 发布保留上一版哈希资源，避免旧缓存页面在切换期间加载失败。上一版回滚地址：`https://github.com/Asakushen/cfsm-theme-nagisa/tree/ceae75b990acf2b71a73b3e08221edc3851f33e5`。
